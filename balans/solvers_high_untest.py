import math
import random
from typing import Tuple, Dict, Any, List, Sequence, Mapping
...
class HighsSolver(_BaseMIP):
    ...
    def __init__(self, instance_path: str, seed: int):
        ...
        if status != HighsStatus.kOk:
            raise RuntimeError(
                f"HiGHS failed to read '{instance_path}' (status={status})."
            )
    ...
    def get_obj_value(self, index_to_val: Dict[int, float]) -> float:
        return sum(
            self.org_coeffs[i] * index_to_val.get(i, 0.0)
    ...
    def fix_vars(
        self,
        idx_to_val: Dict[int, float] | None,
    ...
    def proximity(self, idx_to_val, obj_val, delta: float, binary_idx):
        ...
        self.constraints.append(
            self.model.addConstr(self._objective_expression() <= rhs)
        )
    ...
    def solve_and_undo(
        self,
        time_limit_sc: float | None = None,
        sol_limit: int | None = None,
    ):
    ...
    def _reset_after_solve(self, time_limit_sc, sol_limit):
        if time_limit_sc is not None:
            self.model.setOptionValue("time_limit", kHighsInf)
        if sol_limit is not None:
            self.model.setOptionValue("solution_limit",
